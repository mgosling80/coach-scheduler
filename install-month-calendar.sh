#!/usr/bin/env bash
# install-month-calendar.sh
# Replaces the week calendar with a full month grid view.
# Run from project root: bash install-month-calendar.sh
...
echo "Writing src/app/coach/schedule/schedule-client.tsx (month view)"
cat > src/app/coach/schedule/schedule-client.tsx << 'FILE_EOF'
'use client';
...
# Widen the page container so the month calendar has room to breathe.
echo "Widening the coach layout container"
sed -i.bak 's|max-w-6xl|max-w-7xl|g' src/app/coach/layout.tsx
rm -f src/app/coach/layout.tsx.bak

echo ""
echo "Done. Month calendar view installed."